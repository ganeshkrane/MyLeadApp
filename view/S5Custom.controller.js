...
sap.ui.controller("cus.crm.opportunity.CRM_OPPRTNTYExtension.view.S5Custom", {
	...
	resetFunc2:function(){
		...
		this.byId("cusDistrict").setValueState("None");
		this.byId("cusCity").setValueState("None");
		this.byId("cusState").setValueState("None");
		this.byId("cusPhone").setValueState("None");
		this.byId("cusStd").setValueState("None");
	},
	...
	dataConfirm: function(r) {
		...
		var ExstCusFlg = this.byId("CustSw_id").getState() ? "X" : "";
			...
			var buaggTab=this.byId("busiAggrements_tblId");
			var buaggNumTab=this.byId("busiAggrements");
			var Items = ProductTab.getItems();
			if (Items.length > 0 && ProductTab.getVisible()) {
			...
			}else if(buaggTab.getVisible()){
					var Items = buaggTab.getSelectedItems();
					if (Items.length > 0) {
				for (var i = 0; i < Items.length; i++) {
					var _saveingRecord = {
						"Guid": "00000000-0000-0000-0000-000000000001", //_zProduct.results[i].Guid,
						"RecordId": "00000000-0000-0000-0000-000000000001",
						"Buag": Items[i].getCells()[0].getText()
					};
					E.ZProduct.push(_saveingRecord);
				}
			}
			}else if(buaggNumTab.getVisible()){
					//var Items = buaggTab.getItems();
			//		if (Items.length > 0) {
			//	for (var i = 0; i < Items.length; i++) {
					var _saveingRecord = {
						"Guid": "00000000-0000-0000-0000-000000000001", //_zProduct.results[i].Guid,
						"RecordId": "00000000-0000-0000-0000-000000000001",
						"Buag": this.byId("BuagInput").getValue()
					};
					E.ZProduct.push(_saveingRecord);
			//	}
		//	}
		...
		/*	m.create("/Opportunities", E, null, function(O, Q) {
			...
			m.submitBatch(function(O, Q) {
				//	m.create("/Opportunities", E, null, function(O, Q) {
				if (O.__batchResponses[0].response.statusCode === "400") {
	...
	_onPostalReadSuccess: function(d, oResponse) {
		...
		if (d.results.length === 0) {
			...
			this.getView().byId("cusCity").setValue(d.results[0].City1);
	...
	showAccountF4: function(e) {
		...
		var t = this;
			this.accountF4Fragment.attachBrowserEvent("keypress", function(oEvent) {
				var _keyCode = (oEvent.keyCode ? oEvent.keyCode : oEvent.which);
				if (_keyCode === 13) {
					t.onBPSearch();
				}
			});
	...
	setAccount: function(e) {
		...
		//this._setBuagDetails(this.accountId);
...
_setBuagDetails:function(accId){
	var t=this;
	if(this.byId("leadType").getSelectedKey() === "T" && this.byId("CustSw_id").getState()){
		var sFilter1 = new sap.ui.model.Filter("Partner", "EQ", accId);
		this.byId("busiAggrements_tblId").setModel("MultiSelect");
	
	}else{
		var sFilter1 = new sap.ui.model.Filter("Buag", "EQ", accId.getSource().getValue());
	 }
	//this.byId("busiAggrements_tblId").setVisible(true);
	this.oModel.read("/ZBUAGF4Set", {
				filters: [sFilter1],
				success: jQuery.proxy(this._BuagReadSuccess, this),
				error: jQuery.proxy(this._onBuagReadError, this)
	});
},
_BuagReadSuccess:function(d){
//var json=new sap.ui.model.json.JSONModel(d);
//this.byId("busiAggrements_tblId").setModel(json);
if(this.byId("leadType").getSelectedKey() === "T" && this.byId("CustSw_id").getState()){
	        this._custData.ZBuagData = d;
			this._custJsonModel.setData(this._custData);
			this._custJsonModel.updateBindings();
}
	...
	extHookOnSave: function() {
		...
		var buaggTabItems=this.byId("busiAggrements_tblId").getItems();
		var buaggTabSelItems=this.byId("busiAggrements_tblId").getSelectedItems();
		var buaggNumTab=this.byId("BuagInput");
		
		var Items = prodTab.getItems();
		if (Items.length === 0 && this.byId("idProdTable").getVisible()) {
			sap.m.MessageToast.show("Select atleast one Product and Variant");
			return false;
		}else if(buaggTabItems.length > 0 && buaggTabSelItems.length === 0 &&  this.byId("busiAggrements_tblId").getVisible()){
			sap.m.MessageToast.show("Select atleast one Business Agreement Number");
			return false;
		}else if(buaggNumTab.getValue()==="" &&  this.byId("busiAggrements").getVisible()){
			sap.m.MessageToast.show("Enter Business Agreement Number");
			buaggNumTab.setValueState("Error");
			return false;
		} /*else {
	...
	_onExistingCustSwich: function(oEvent) {
		...
		//	this.byId("BusAggr").setValue();
			...
			this.byId("cusDealerLocation").setValue("");
			/////////// set value state error ///////////
			this.byId("busiAggrements").setVisible(false);
		...
		this.byId("statusdropdown").removeStyleClass("controlValueStateError");
		this.byId("cusMobile").setValueState(sap.ui.core.ValueState.None);
		this.byId("cusAdd1").setValueState(sap.ui.core.ValueState.None);
		this.byId("cusAdd2").setValueState(sap.ui.core.ValueState.None);
		this.byId("cusAdd3").setValueState(sap.ui.core.ValueState.None);
	...
	statusdropdownChnge:function(evt){
		evt.getSource().removeStyleClass("controlValueStateError");
	},
	validateCustomSavePage: function() {
		...
		if (this.byId("cusCity").getValue() === "" && this.byId("cusCity").getVisible() === true) {
		...
		//	this.byId("statusdropdown").setValueState(sap.ui.core.ValueState.Error);
			this.byId("statusdropdown").addStyleClass("controlValueStateError");
			c = true;
		}
	...
	onMandatoryFieldChanged: function(e) {
		...
		if (e.mParameters.id.indexOf("BuagInput") > -1) {
			var val = e.mParameters.value;
			val = val.replace(/[^0-9 ]/g, '');
			e.getSource().setValue(val);
	...
	onComboChanged: function(e) {
		...
		if (e.mParameters.id.indexOf("leadType") > -1) {
			...
			//	this.byId("BusAggr").setValue();
				...
				this.byId("busiAggrements_tblId").removeSelections(false);
				this.byId("busiAggrements_tblId").setVisible(true);
				this.byId("busiAggrements_tblId").setMode("MultiSelect");
	...
	onBranchChanged: function(e) {
		...
		if (this.branch !== "" && this.distchannel !== "" && this.byId("busiAggrements_tblId").getVisible() === false) {
	...
	onApplicantChanged: function() {
		...
		if (this.byId("applicantType").getSelectedKey() === "2" || this.byId("applicantType").getSelectedKey() === "3") {
			...
			if (this.byId("applicantType").getSelectedKey() === "3") {
				...
				this.byId("cusMobile").setVisible(false);
				//	this.getView().byId("Name2").setRequired(true);
				if(!this.byId("CustSw_id").getState()){
				this.byId("busiAggrements").setVisible(true);
				this.byId("BuagInput").setValue();
				this.byId("busiAggrements_tblId").setVisible(false);
				this.byId("idProdTable").setVisible(false);	
	...
	handleDetermination: function() {
		if (!jQuery.isEmptyObject(this.oPrevAccount) && this.oPrevAccount.accountId !== this.accountId) {
			...
			sap.m.MessageBox.show(sap.ca.scfld.md.app.Application.getImpl().getResourceBundle().getText('DETERM_LOSS'), {
				icon: sap.m.MessageBox.Icon.WARNING,
				title: sap.ca.scfld.md.app.Application.getImpl().getResourceBundle().getText('WARNING'),
				actions: [sap.ca.scfld.md.app.Application.getImpl().getResourceBundle().getText('CONTINUE'), sap.m.MessageBox.Action.CANCEL],
				onClose: jQuery.proxy(function(a) {
					if (a == "Continue") {
